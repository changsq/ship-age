<template>
    <div>
        <div class="pie2">
            <div class="title">
                <dv-decoration-7 style="width:150px;height:30px;margin-left: 150px;color: white;">备件数据</dv-decoration-7>
            </div>
            <div v-if="!showInfo" style=" font-size:large;color: white;position: absolute; margin: 50%;">当前无数据</div>
            <div v-if="showInfo" class="percentage">
                <table>
                    <thead>
                        <tr>
                            <th>部件名称</th>
                    ...
                    <tbody>
                        <tr v-for="(item, index) in tableData" :key="index">
                            ...
                            <td v-show="item.subElementType != ''" @click="checkInfo(index)"><a-icon type="info-circle"
                                    theme="twoTone" /></td>
                        </tr>
                    </tbody>
                </table>
            </div>
            <div v-if="showInfo" style="display: flex; margin-top: 580px;justify-content: center">
                <a-pagination :current="pageNumber" :total="total" @change="onPageChange" />
            </div>
            <div class="pie2-footer"></div>
        </div>
        <a-modal v-model="open" title="详细信息" :footer="[]">
            <div style="margin-left: 20px;">
                <p>名称：{{ tableData[index] && tableData[index].elementType }}</p>
                <p>型号/规格：{{ tableData[index] && tableData[index].modelSpecification }}</p>
                <p>库存：{{ tableData[index] && tableData[index].inventory }}</p>
                <p>生产商：{{ tableData[index] && tableData[index].manufacturer }}</p>
                <!-- <p>价格：{{ tableData[this.index].price }}</p>
            <p>建议：{{ tableData[this.index].sparePartAdvice }}</p>
            <p>备注：{{ tableData[this.index].remark }}</p> -->
            </div>
        </a-modal>
    </div>
...
export default {
    ...
    methods: {
        ...
        filterArray(array, value) {
            if (array) {
                if (array.filter(item => item.elementName.includes(value)).length != 0) {
                    ...
                    return [{ elementName: '', sparePartAdvice: '' }]
                };
            }
        },
        ...
        fetchData() {
            this.queryData('/hdapi/RULserver/spareParts/getSparePartsList', {
